<template>
  <div class="pagedesign">
      ...
      <el-tabs type="border-card" tab-position="left" style="height: 98vh">
        <el-tab-pane label="文本">
          <div class="text_selection">
            ...
            <div class="text2" @click="addtext(2)">标题</div>
            <div class="text3" @click="addtext(3)">副标题</div>
            <div class="text4" @click="addtext(4)">正文内容</div>
          </div>
        </el-tab-pane>
              ...
              <div class="size_value">
                <div class="size_x sizeDo">
                  ...
                  <el-input
                    ...
                    @input="
                      inputChange(
                        view[select_index].left,
                        view[select_index].top
                      )
                    "
                  ></el-input>
                </div>
    ...
    >
      <!-- <span>{{ finalData }}</span> -->
      <img
        style="margin: auto; display: block; border: 1px solid #eee"
        :src="imgUrl"
        alt=""
      />
...
function addtext(i) {
  ...
  view.push(obj);
  view[view.length - 1].z_index = view.length;
  view[view.length - 1].id = id.value;
  id.value++;
}
// 添加图形
function addshape(i) {
...
function onResize(x, y, width, height) {
  ...
  view[index].top = y;
  view[index].left = x;
  view[index].width = width;
  view[index].height = height;
}
// 拖动组件
function onDrag(x, y) {
  const index = select_index.value;
...
function exportJson() {
  ...
  const list = JSON.parse(JSON.stringify(view));
